...
Guidance for AI coding agents (Claude, Cursor, Copilot, Codex, etc.) working in this repository. Humans should also find this useful as a quick orientation; for the full developer setup see [DEVELOPMENT.md](./DEVELOPMENT.md), and for PR conventions see [CONTRIBUTING.md](./CONTRIBUTING.md).

> **Tool-specific config files redirect here.** [`CLAUDE.md`](./CLAUDE.md), [`.cursor/rules/project.mdc`](./.cursor/rules/project.mdc), and [`.github/copilot-instructions.md`](./.github/copilot-instructions.md) are stubs that point back to this file so we maintain guidance in one place. If you're adding new agent guidance, edit `AGENTS.md` rather than the stubs.
...
- **Language:** Go (see `.go-version` and `.tool-versions` for exact versions).
...
GNUmakefile                      # build / test / test-unit / testacc / generate / fmt / vet / errcheck
.go-version / .tool-versions     # Go + golangci-lint versions (asdf/mise compatible)
.editorconfig                    # Whitespace/indent rules per file type
.pre-commit-config.yaml          # gofmts + golangci-lint + make-generate-no-change hook
.goreleaser.yml                  # Cross-platform release builds
release-please-config.json       # Automated changelog + version bumps
AGENTS.md / CONTRIBUTING.md / DEVELOPMENT.md / README.md
CLAUDE.md / .cursor/rules/ / .github/copilot-instructions.md  # Stubs pointing here
```
...
9. **PR titles must be [Conventional Commits](https://www.conventionalcommits.org/).** Enforced by `.github/workflows/lint-pr-title.yml` and consumed by [release-please](https://github.com/googleapis/release-please) for automated changelog and version bumps. See [CONTRIBUTING.md](./CONTRIBUTING.md) for examples.
...
| `make test-unit` | **Recommended for tight agent loops.** Runs `go test -skip '^TestAcc' ...` — only non-acceptance tests, no LaunchDarkly API calls, safe to run anywhere. |
| `make test` | Same as `test-unit` plus skipped acceptance tests in the output. Acceptance tests `t.Skip()` themselves when `TF_ACC` is unset. |
...
- **Don't run `make testacc` in a feedback loop.** Use `make test-unit` instead — `testacc` creates real LaunchDarkly resources and requires enterprise credentials.
...
- **Don't edit the tool-specific stubs (`CLAUDE.md`, `.cursor/rules/`, `.github/copilot-instructions.md`)** to add guidance. Edit `AGENTS.md` and let the stubs continue to redirect here.
