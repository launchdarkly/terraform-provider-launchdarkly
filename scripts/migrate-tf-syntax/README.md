...
- `blocks` — attributes that switched from block to nested attribute. Nested entries describe attributes inside an element that themselves migrated (e.g. `rules` contains `clauses`). Set `"object": true` for a genuine single-object attribute (`SingleNestedAttribute`): forward emits `name = { ... }` (no brackets) and reverse parses that object back to a block. Omit it for the default list-of-objects shape. LD's single-object attributes are `client_side_availability`, `defaults`, `default_client_side_availability`, `fallthrough`, `approval_settings`, `segment_approval_settings`, and `instructions` (REL-14237). Set `"map_key": "<inner attr>"` for a `MapNestedAttribute` keyed by one inner field (`environments` on project, `custom_properties` on feature_flag — REL-14236): forward emits `name = { <keyval> = { ...rest } }`, hoisting that field to the map key (the field also stays inside the object); reverse expands the map back to repeated blocks, re-injecting `<inner attr> = <key>` when absent. Add `"map_value": "<inner attr>"` (with `map_key`) for a plain map attribute (`role_attributes` on team/team_member): forward collapses each block to `name = { <keyval> = <value-expr> }` using the named attribute's expression as the map value; reverse expands each entry back to a `{key, values}` block. Literal-string keys hoist verbatim; a non-literal key (a `var`/`local`/function call) hoists as a parenthesized-expression map key (`(local.env_key) = {...}`) with a stderr `note:`. Duplicate key text (literal or expression) aborts the attribute with a warning. `object` and `map_key` are mutually exclusive.
...
2. **Limited semantic upgrades.** The script converts syntax and fills one deterministic gap: it synthesizes the required `variations` on a `launchdarkly_feature_flag` whose `variation_type` is the literal `"boolean"` and that omitted variations, writing the LaunchDarkly invariant `[{ value = "true" }, { value = "false" }]`. It does **not** set variation `name`/`description` — it cannot see values set outside the config — but it does not need to: provider v3 preserves any variation `name`/`description` set outside Terraform when the config omits them. It does not synthesize variations for multivariate flags or when `variation_type` is a non-literal expression (a var/local) — it warns and leaves those for you.
...
A `types.List` / `types.Set` paired with a `ListNestedAttribute` / `SetNestedAttribute` means list-of-objects; add it to `mappings.json` without `object`. A `types.Object` paired with a `SingleNestedAttribute` means single object; add it with `"object": true`. A `types.Map` paired with a `MapNestedAttribute` means a key-addressed map; add it with `"map_key": "<inner attr that became the map key>"`. A `types.Map` paired with a plain `MapAttribute` means the object collapsed entirely; add it with both `"map_key"` and `"map_value"`.
