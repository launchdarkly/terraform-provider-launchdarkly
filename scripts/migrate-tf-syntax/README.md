...
2. **Limited semantic upgrades.** The script converts syntax and fills one deterministic gap: it synthesizes the required `variations` on a `launchdarkly_feature_flag` whose `variation_type` is the literal `"boolean"` and that omitted variations, writing the LaunchDarkly invariant `[{ value = "true" }, { value = "false" }]`. It does **not** set variation `name`/`description` — it cannot see values set outside the config, and the provider rewrites variation name/description from config on apply, so if a boolean flag has those set out-of-band, add them before applying or the first apply clears them. It does not synthesize variations for multivariate flags or when `variation_type` is a non-literal expression (a var/local) — it warns and leaves those for you.
