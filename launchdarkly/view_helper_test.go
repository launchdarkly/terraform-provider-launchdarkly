...
func TestValidateViewKeysExist(t *testing.T) {
	...
	// validateViewKey rejects uppercase keys at plan time, so this input can no
	// longer arrive from a plan. The helper still must not lowercase it on the
	// caller's behalf: it looks up exactly what it was handed, so a key reaching
	// it by any other path (import, drift reconcile) fails loudly instead of
	// silently resolving to a different view.
